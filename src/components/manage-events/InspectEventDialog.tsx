...
import { Clock, Info, ListCollapse, ScrollText, Search, Trophy, Users, Wallet } from 'lucide-react';
...
export const InspectEventDialog = ({ event }: InspectEventDialogProps) => {
    ...
    const [activeTab, setActiveTab] = useState<'about' | 'rules' | 'details'>('about');
                            ...
                            <div className="mt-0 focus-visible:outline-none outline-none">
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    <div className="p-4 bg-white/5 rounded-xl border border-white/10 flex flex-col gap-1">
                                        <span className="text-xs text-zinc-400 flex items-center gap-1.5"><Wallet className="w-3.5 h-3.5 text-violet-400" /> Registration Fee</span>
                                        <span className="text-lg font-semibold text-white">₹{event.registration_fees}</span>
                                    </div>
                                    <div className="p-4 bg-white/5 rounded-xl border border-white/10 flex flex-col gap-1">
                                        <span className="text-xs text-zinc-400 flex items-center gap-1.5"><Trophy className="w-3.5 h-3.5 text-amber-400" /> Prize Pool</span>
                                        <span className="text-lg font-semibold text-white">₹{event.prize_pool}</span>
                                    </div>
                                    <div className="p-4 bg-white/5 rounded-xl border border-white/10 flex flex-col gap-1">
                                        <span className="text-xs text-zinc-400 flex items-center gap-1.5"><Users className="w-3.5 h-3.5 text-blue-400" /> Team Size</span>
                                    ...
                                    <div className="p-4 bg-white/5 rounded-xl border border-white/10 flex flex-col gap-1">
                                        <span className="text-xs text-zinc-400 flex items-center gap-1.5"><Info className="w-3.5 h-3.5 text-emerald-400" /> Registration Status</span>
