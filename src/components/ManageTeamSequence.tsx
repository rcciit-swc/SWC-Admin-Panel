...
function SortableItem({ member }: { member: TeamMember }) {
    ...
    return (
        <div
            ref={setNodeRef}
            style={style}
            className={`flex items-center gap-4 bg-white/5 border border-white/10 rounded-xl p-4 transition-all ${isDragging ? 'opacity-50 scale-105 shadow-2xl z-50' : 'hover:bg-white/10'
                }`}
        >
            {/* Drag Handle */}
            <button
            ...
            <div className="flex-shrink-0 w-10 h-10 bg-indigo-600/30 border border-indigo-500/50 rounded-lg flex items-center justify-center">
                <span className="text-indigo-300 font-bold text-sm">{member.sequence}</span>
            </div>
...
export default function ManageTeamSequence({ isSuperAdmin }: ManageTeamSequenceProps) {
    ...
    const handleSaveChanges = async () => {
        ...
        try {
            ...
            toast.success(`Successfully updated ${teamsToUpdate.length} team${teamsToUpdate.length > 1 ? 's' : ''}!`);
            ...
            if (error?.message?.includes('Access denied') || error?.message?.includes('super_admin')) {
                toast.error('Access denied. Only super admins can update team sequences.');
            } else {
            ...
            <div className="max-w-6xl mx-auto">
                ...
                <div className="bg-gradient-to-br from-indigo-950/40 to-purple-950/40 border border-white/10 rounded-2xl p-6 mb-6">
                    ...
                    <p className="text-zinc-400">
                        Drag and drop to reorder team members. Click "Save Changes" when done.
                    </p>
                </div>
                ...
                {hasChanges && (
                    <div className="mb-6 bg-amber-950/40 border border-amber-500/30 rounded-xl p-4 flex items-center justify-between">
                        <div>
                            <p className="text-amber-300 font-semibold">You have unsaved changes</p>
                            <p className="text-zinc-400 text-sm">Click "Save Changes" to update the sequences</p>
                        </div>
                        <Button
