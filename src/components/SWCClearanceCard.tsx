...
export default function SWCClearanceCard({ defaultRollNumber }: SWCClearanceCardProps) {
            ...
            <CardContent className="space-y-4">
                {!defaultRollNumber ? (
                    <div className="p-4 rounded-lg bg-yellow-500/10 border border-yellow-500/20 text-yellow-200 text-sm">
                        Please complete your profile and save your <strong>College Roll Number</strong> below to check your SWC Clearance status.
                    </div>
                ) : (
                    ...
                    <div className="p-4 rounded-lg bg-emerald-500/10 border border-emerald-500/20 flex items-start gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-sm text-zinc-300 mt-1">
                                Student Name: <span className="text-white font-medium">{result.data.name}</span>
                            </p>
                            <p className="text-sm text-zinc-300">
                                Department: <span className="text-white font-medium">{result.data.department}</span>
                            </p>
                            <p className="text-sm text-zinc-300">
                                Payment Status: <span className="text-white font-medium">Success</span>
                            </p>
                        </div>
                    </div>
                )}
                ...
                {result.status === 'not_found' && (
                    <div className="p-4 rounded-lg bg-red-500/10 border border-red-500/20 flex items-start gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-sm text-zinc-300 mt-1">
                                No verified record found for this Roll Number. Please ensure you have paid the SWC funds and your data is updated in the sheet.
                            </p>
                        </div>
                    </div>
                )}
